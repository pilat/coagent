...
- **Operator progress + one-shot budgets**: model-bound input acceptance stays in the durable inbox without a chat receipt. A manager-owned root's autonomous episode starts with initial/reactivating model input or an applied scheduled turn; queued input keeps the clock, while empty/read-only roots have none. Meaningful changes and five-minute silence enqueue a replaceable canonical snapshot; `/status` renders it explicitly, and final answers append only non-empty TODO/budget footer parts. A user-authorized `/budget` can stop one root tree on additional persisted cost or wall time and is released by the next ordinary root input

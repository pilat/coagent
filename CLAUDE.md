...
- **`internal/budget`** — One-shot root-tree cost/duration checkpoint policy, validation, timers, and its user-authorized tool
...
- **`internal/progress`** — Transport-neutral operator progress snapshots and compact/full/final Markdown rendering
...
- **`internal/configtools`** — Agent-facing provider/model/manager tool schemas, strict argument parsing, and mapping to `configops` operations; the daemon decides where the tools exist and owns their suspend/restart handoff
...
- **Telegram file ingestion + vision**: uploads (Document/Photo/Video/Audio) are saved verbatim to `/tmp` under random names and enter the session as a fixed-EN synthetic metadata text whose only conditional advice points small images at `read`; no attachment payload crosses any manager boundary. `read` accepts {jpeg, png, gif, webp} ≤5 MB and attaches a disk reference (`messages.attachments`) that drivers re-materialize per request, gated fail-closed on catalog-provided input modalities ([ADR-0034](docs/adr/0034-vision-via-referenced-tool-result-attachments.md))
...
- **Append-only context log**: Stored message content is immutable after insert. Context transformation is metadata (`compacted_at`) plus appended rows; the rendered message list is a projection computed at load, so the prompt prefix is byte-stable between events (cache-friendly)
- **Compaction is the only automatic pressure response** ([ADR-0035](docs/adr/0035-compaction-summarizes-a-bounded-head.md)): no continuous pruning, no clearing stage, no decisions keyed on prompt-cache state. It runs at exactly one point in the loop (`applyContextEvents`, where nothing is pending) and summarizes only a bounded older head — one no-tools model call over the repaired canonical JSONL projection — while a repair-free verbatim tail of at least a tenth of the window survives when that much history exists. The committed checkpoint rebuilds the transcript as header → marked summary → optional current-skill envelope → exact tail; a failed, empty, non-relieving or length-stopped attempt changes nothing. `/compact [focus]` and automatic pressure are the only triggers — behind a non-sleep pending call the request waits in the durable inbox
...
- **Operator progress + one-shot budgets**: model-bound input acceptance stays in the durable inbox without a chat receipt, and each promotion/scheduled injection advances a durable model-input generation that delimits replaceable output — manager replay edits previous external messages only across equal adjacent generations. A manager-owned root's autonomous episode starts with initial/reactivating model input or an applied scheduled turn; queued input keeps the clock, while empty/read-only roots have none. Meaningful changes and five-minute silence enqueue a replaceable canonical snapshot (note-first, TODO counts only) that a stale capture drops as superseded; `/status` renders it explicitly, and final answers append only non-empty TODO summary/budget footer parts. Explicit `/stop` is a durable two-phase user-visible operation — replaceable `⏳ Stopping…` at the fence, then one terminal transaction releasing the armed budget, the `stopped` status, and `⏸️ Session stopped`. A user-authorized `/budget` can stop one root tree on additional persisted cost or wall time and is released by the next ordinary root input
...
- **A model entry has an ID, provider, and optional user-defined tags.** `name`, `context_window`, `max_tokens` and `pricing` were removed from the schema — they come from the provider's catalog at startup, and an old config carrying them fails loudly. Tags use `^[a-z0-9_-]+$` and bound autonomous `task` choices without restricting human `/model` selection. `TimeoutSec` and `openrouter_config` stay (behavior knobs, not metadata). A provider may set `catalog: <models.dev section>` when its driver name does not identify the vendor; `anthropic` defaults to `anthropic`, `google-sa` to `google-vertex`, and a bare `openai` provider searches every section in sorted order.

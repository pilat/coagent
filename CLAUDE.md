...
- **Onboarding**: bare `coagent` is a deterministic bootstrap (install-or-update the daemon, collect one provider and its key at a masked prompt over the control socket) followed by an AI-led chat served by the built-in CLI manager. Everything past the first provider is configured by asking the agent, which has tools for it
...
- **Skills**: Model discovery and direct user invocation are independent. `disable-model-invocation: true` removes a skill from the prompt/tool inventory; `user-invocable: false` rejects `/skill <name>`. A leading `/skill <name> [args]` expands programmatically before the LLM call
...
## Code Navigation (goscope)

`goscope` is a structural navigator for this Go module — a fully type-checked in-memory
index answering "who depends on what, what breaks if I change this" with exact `file:line`
positions. Precise where `grep` guesses. Read-only, auto-spawns a warm daemon on first
query, re-indexes on file change. Run it from the repo root; a local-dev aid, not present
in CI/containers. When it's on your PATH, prefer it over grep for structural questions.

**Use it for:**
- `goscope blast_radius <pkg> <sym>` — what breaks if I change/remove a symbol (aggregate: refs + impls + transitive importers)
- `goscope impls <pkg> <iface>` — who implements an interface; `satisfies <pkg> <type>` — which interfaces a type satisfies
- `goscope refs <pkg> <sym>` — every use; `describe <pkg> <sym>` — signature, doc, ref count
- `goscope graph <pkg> [depth]` — imports / imported-by; `usages <importer> <imported>` — which symbols make an edge exist
- `goscope symbols <pkg>` — everything a package declares
- `goscope check` — does the module still type-check — **run after every edit**

**Refactor loop:** `blast_radius` (see the reach) → edit → `check` (confirm nothing broke).

`<pkg>` is a full import path, module-relative path, or unique suffix (`internal/tool`
≡ `tool`). `<sym>` is a bare identifier (`Tool`) or a dotted method/field
(`Tool.Run`, exactly one dot). Positions are 1-based; `inTest`/`generated` flag test and
generated files. Run `goscope` with no args for the full manual.

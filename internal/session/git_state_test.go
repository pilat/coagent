...
func TestAppendGitStateDelta_FailedProbeCarriesBoundedReason(t *testing.T) {
	gc := &fakeRepoStateClient{err: errors.New("git status probe: context deadline exceeded")}
	s := newDeltaSession(t, t.TempDir(), gc, &promptRecordingLLM{})

	out := s.appendGitStateDelta(context.Background(), "user text")

	report, ok := gitDelta(out)
	require.True(t, ok, "a failed probe must still send a delta so the model knows its state is stale")
	assert.Equal(t, "Git state: unavailable: git status probe: context deadline exceeded", report)
}

func TestAppendGitStateDelta_ProbeReasonIsRedactedAndBounded(t *testing.T) {
	secret := "ghp_supersecrettokenvalue"
	long := "git status probe: " + strings.Repeat("x", 500) + " tail"
	s := newDeltaSession(t, t.TempDir(), &fakeRepoStateClient{err: errors.New(long)},
		&promptRecordingLLM{})
	logger.AddRedactedValues(secret)

	out := s.appendGitStateDelta(context.Background(), "user text")

	report, ok := gitDelta(out)
	require.True(t, ok)
	assert.NotContains(t, report, secret, "secrets must be redacted from the model-facing reason")
	assert.LessOrEqual(t, len([]rune(report)), 22+len("…")+255, "the reason must stay bounded")
	assert.Contains(t, report, "…")
}

func TestAppendGitStateDelta_NilClientSendsNothing(t *testing.T) {
	s := newDeltaSession(t, t.TempDir(), nil, &promptRecordingLLM{})

	assert.Equal(t, "user text", s.appendGitStateDelta(context.Background(), "user text"))
}

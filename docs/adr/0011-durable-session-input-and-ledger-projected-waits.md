...
- Persist user, agent and independent process/subagent input in the SQLite `session_inbox` FIFO before acknowledging it.
...
- Serialize each child's follow-up/terminal boundary. Follow-up accepted before terminalization stays in the current activation. Follow-up after terminalization starts a new activation only after the previous outcome was durably handed to the parent inbox. Continuation is available for both foreground and background children; once a foreground task result has resolved its original call, later activations report through `<subagent_completion>` user-role input.
...
- `/stop` parks the root and all live descendants, resolves outstanding calls with a stopped result, removes exact pending-sleep rows, cancels ordinary accepted-but-unconsumed input, and preserves already-pending process/subagent facts plus recurring and standalone one-shot schedules. Sessions and child history remain available; a stopped child resumes only through explicit follow-up. `/kill` remains destructive and cancels pending asynchronous facts.
